...
private:
    ...
    AcquisitionPeak find_highest_peak(uint32_t num_doppler_bins,
        const std::vector<uint32_t>& excluded_code_phases,
        int32_t only_doppler_bin = -1) const;
    AcquisitionPeak find_selected_path_peak(uint32_t num_doppler_bins,
        std::vector<uint32_t>& excluded_code_phases) const;
    AcquisitionResult make_result(const AcquisitionPeak& peak,
        int32_t doppler_max, int32_t doppler_step) const;
    bool code_phase_is_excluded(uint32_t index_time,
    ...
    const uint32_t d_second_peak_exclusion_samples;
