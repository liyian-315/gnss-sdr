...
void pcps_acquisition::log_acquisition(const AcquisitionResult& result) const
{
    ...
    if (d_acq_parameters.multipath_detection && result.has_second_peak)
        {
...
void pcps_acquisition::send_positive_acquisition(const AcquisitionResult& result)
{
    ...
    // Multipath survey: when second-path detection is on, report the second correlation
    // peak for every acquired satellite (delay of the reflection relative to the main peak,
    // and their power ratio in dB). Lets a single run flag which satellites see multipath.
    if (result.has_second_peak)
        {
            const float spc = (d_samplesPerChip > 0) ? static_cast<float>(d_samplesPerChip) : 1.0F;
            const float main_chips = static_cast<float>(result.index_time) / spc;
            const float second_chips = static_cast<float>(result.index_time2) / spc;
...
void pcps_acquisition::dump_results(const AcquisitionResult& result)
{
    ...
    if (matfp == nullptr)
    ...
    else
        {
            ...
            if (d_acq_parameters.multipath_detection)
                {
                    write_matlab_var<1>("has_second_peak", static_cast<int32_t>(result.has_second_peak ? 1 : 0), matfp, dims_1d);
                    write_matlab_var<1>("acq_delay_samples_2", static_cast<float>(std::fmod(static_cast<float>(result.index_time2), d_acq_parameters.samples_per_code)), matfp, dims_1d);
...
void pcps_acquisition::find_second_peak(AcquisitionResult& result)
{
    // Locate the Doppler bin and code-phase index of the main (strongest) peak.
    float grid_maximum = 0.0;
    uint32_t index_doppler = 0U;
    ...
    for (uint32_t i = 0; i < d_num_doppler_bins; i++)
        {
            volk_gnsssdr_32f_index_max_32u(&tmp_index, d_magnitude_grid[i].data(), d_effective_fft_size);
            if (d_magnitude_grid[i][tmp_index] > grid_maximum)
                {
                    grid_maximum = d_magnitude_grid[i][tmp_index];
                    index_doppler = i;
                    index_time = tmp_index;
                }
        }
...
void pcps_acquisition::update_synchro(const AcquisitionResult& result)
{
    ...
    d_gnss_synchro->Acq_delay_samples = static_cast<double>(std::fmod(static_cast<float>(acq_code_phase), d_acq_parameters.samples_per_code));
    d_gnss_synchro->Acq_doppler_hz = static_cast<double>(acq_doppler);
