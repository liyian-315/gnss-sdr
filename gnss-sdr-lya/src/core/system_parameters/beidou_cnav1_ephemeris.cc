 ...
 * \brief  Interface of a BEIDOU CNAV1 EPHEMERIS storage and orbital model functions
...
Beidou_Cnav1_Ephemeris::Beidou_Cnav1_Ephemeris()
{
	// Other values
	i_satellite_PRN = 0;
	//SOW = 0.0;
	...
	ISC_B1Cd = 0.0;			//Group delay differential between the B1C data and pilot components [s]
...
double Beidou_Cnav1_Ephemeris::B1c_ranging_code_phase_correction_w_pilot(double dt_sv)
{
	double dt_sv_B1Cp;
	dt_sv_B1Cp = dt_sv - T_GDB1Cp;
	return dt_sv_B1Cp;
}

double Beidou_Cnav1_Ephemeris::B1c_ranging_code_phase_correction_w_data(double dt_sv)
{
	double dt_sv_B1Cd;
	dt_sv_B1Cd = dt_sv - T_GDB1Cp - ISC_B1Cd;
	return dt_sv_B1Cd;
}

double Beidou_Cnav1_Ephemeris::B1c_UT1_UTC_difference(double t)
{
	return dUT1 + dUT1_dot*(t-t_EOP);		//WARNING:ERROR  Changed to "+" Original:"-"
}

double Beidou_Cnav1_Ephemeris::B1c_Polar_motion_x(double t)
{
	double x_p;
...
double Beidou_Cnav1_Ephemeris::B1c_Polar_motion_y(double t)
{
	double y_p;
