...
#ifndef GNSS_SDR_BEIDOU_CNAV1_EPHEMERIS_H_
#define GNSS_SDR_BEIDOU_CNAV1_EPHEMERIS_H_
...
/*!
 * \brief This class is a storage and orbital model functions for the BEIDOU SV ephemeris data as described in Beidou B1C ICD (Edition 5.1)
 */
...
public:
	...
	double ISC_B1Cd;		//Group delay differential between the B1C data and pilot components [s]
    ...
    void serialize(Archive& archive, const unsigned int version)
    {
		...
		archive& make_nvp("ISC_B1Cd", ISC_B1Cd);	//Group delay differential between the B2a data and pilot components [s]
    ...
    double B1c_ranging_code_phase_correction_w_pilot(double dt_sv);
    // The single frequency user processing pseudorange from the B1C pilot component shall further correct the ranging code phase

    double B1c_ranging_code_phase_correction_w_data(double dt_sv);
    // The single frequency user processing pseudorange from the B1C data component shall further correct the ranging code phase

    double B1c_UT1_UTC_difference(double t);
    // UT1-UTC difference at time t

    double B1c_Polar_motion_x(double t);
    // Polar motion in the X-Axis at time t

    double B1c_Polar_motion_y(double t);
