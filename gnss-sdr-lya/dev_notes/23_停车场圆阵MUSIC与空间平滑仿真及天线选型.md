...
**在本次已经完成相同条件 Monte Carlo 比较的 4、6、8 阵元圆阵中，推荐制作
8 阵元均匀圆阵试验样机。**

这里的“推荐”准确含义是：8 阵元是当前三种圆阵中最适合继续验证停车场双源分离的
方案，不等于它已经满足整个停车场的最终覆盖要求。

| 候选圆阵 | 选型意见 | 主要原因 |
|---|---|---|
| 4 阵元 | 不推荐用于 PM-FBSS 双源分离 | 只能可靠形成 3 个相位模态；平滑后不足以同时容纳两维信号子空间和噪声子空间 |
| 6 阵元 | 不推荐作为最终样机 | 虽可运行 PM-FBSS，但除正中心外成功率迅速下降，模态和孔径余量不足 |
| 8 阵元 | 推荐作为圆阵试验样机 | 在两天线正中及偏移 5 m 位置，PM-FBSS-MUSIC/MVDR 均达到 100%；阵列可覆盖 360°，没有 ULA 的整条前后模糊 |
| 12 阵元 | 暂不直接推荐制作 | 少量诊断的角度误差优于 8 阵元，但尚未经过同等 Monte Carlo、近场和误差条件比较，证据不等价 |

### 7.2 为什么不是四阵元圆阵

四阵元圆阵看似满足“四通道”硬件条件，但圆阵相位模态空间平滑需要先把物理阵元转换
成连续虚拟模态。四阵元只能稳定保留 $n=-1,0,1$ 三个模态。两源 MUSIC 至少需要：

$$
\underbrace{2}_{\text{两维信号子空间}}
+\underbrace{1}_{\text{至少一维噪声子空间}}
=3 \text{ 维} .
$$

如果再划分重叠子阵进行空间平滑，有效维数还会下降，因此四阵元圆阵没有工程余量。
这不是提高 SNR 或调整 MUSIC 阈值可以弥补的问题。

### 7.3 为什么当前选择八阵元

八阵元相邻弦长取半波长时，可保留五个较可靠的相位模态，再使用长度为 4 的虚拟子阵：

$$
M_{\mathrm{physical}}=8,
\qquad
N_{\mathrm{mode}}=5,
\qquad
P_{\mathrm{subarray}}=4 .
$$

它比六阵元具有更低的有限阵元模态混叠误差。在本次停车场仿真中，接收机位于两根
发射天线之间时，8 阵元 PM-FBSS-MUSIC 与 PM-FBSS-MVDR 均达到 100%，说明它至少
能覆盖停车场最核心的“双天线重叠区”验证需求。

L5 波长约为：

$$
\lambda=\frac{c}{f_{L5}}
=\frac{299\,792\,458}{1\,176.45\times10^6}
\approx0.255\,\mathrm{m} .
$$

若相邻阵元弦长取 $d=0.5\lambda\approx12.7\,\mathrm{cm}$，八阵元圆阵半径为：

$$
r=\frac{d}{2\sin(\pi/8)}\approx16.7\,\mathrm{cm},
$$

因此试验样机直径约为 33.4 cm。这个尺寸是本次仿真对应的推荐几何规格。

### 7.4 八阵元圆阵仍没有解决什么

八阵元在接收机位于两天线侧方时成功率仍只有 0--15%。这说明当前简单 PM-FBSS
存在绝对方位相关的模态截断和阵列流形失配，不能宣称“八阵元可以覆盖整个停车场”。

当两根发射天线从接收机看来处于同一方位时：
...
任何仅依靠空间角度的圆阵都无法把两路分开，增加阵元也不能恢复不存在的角度差。
这种位置必须再利用码延迟、接收机运动或已知天线位置等信息。

### 7.5 如果硬件只能提供四通道

若接收机硬件最终只能同步采集四路，推荐制作 `2×2` 平面阵，而不是四阵元圆阵。
原因是 `2×2` 平面阵可以直接提供两个空间基线，并与当前已经实现的联合空间-时延
GLRT/ML 模型配合；它不依赖四阵元圆阵不足的相位模态维数。

但必须明确：本报告没有完成 `2×2` 与 8 阵元圆阵在相同停车场条件下的统计对比，
所以这里只是“四通道约束下的结构建议”，不能拿它证明 `2×2` 的性能高于八阵元圆阵。
