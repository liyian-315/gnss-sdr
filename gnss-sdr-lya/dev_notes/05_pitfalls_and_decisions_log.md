...
## 2026-08-10 Claude — doc 19 定稿前定点修正(用户评审,补充上一条)

按用户评审修正 doc 19,与上一条冲突处以本条为准:①"4 源"只是 v1 工程研究
范围上限,不是 8 阵元理论源数上限(理论上限取决于有效阵列维数/相干性/算法);
②删除"MUSIC 小于波束宽即失效"的硬结论,40–60° 仅为第一阶段友好实验几何,
真实分辨边界由 G4 实测;③G0 相位漂移 <5° RMS 降为暂定诊断目标,最终预算由
T3 的 0.5-chip 敏感性仿真反推;④block 层默认保持 MMV `y[b,m,k]`(逐块幅度
独立+共享支撑),块内相干平均仅在相位稳定实证后作可选模式;⑤新增 Kronecker
可分性检查,失配显著时主算法允许直接用 G1 实测联合模板 `q_meas`(T9);
⑥G1–G4 正式估计量仅水平面 az,el 后置为扩展;⑦16.2 cm 仅为 L5 理论参考
半径,算法一律用厂家/实测阵元 XYZ;⑧2 源(Δaz≥30/CN0≥45/功率≥-10 dB)与
3 源(Δaz≥40/CN0≥48)条件全部改标 initial test envelope / provisional
...
## 2026-08-11 Claude — 硬件定稿变更:4 物理阵元双馈,doc 19 v1.1 定点修订

**触发**:最终硬件确认为 4 个物理陶瓷阵元×双馈 = 8 路 RF 进 Y790s,**不是
8 独立空间位置 UCA**;设备窗口仅 20 天;实验环境为地下停车场(DAS 每 30–40 m
一处)。按冻结规则以 doc 19 §13 定点修订,不架构性重写。

**要点**:①空间自由度 M=4;第二馈电只作 feed 维保存(rx_mapping:
element_id/feed_id/rx_channel),禁止当第 5–8 个空间阵元,不做极化算法;
②全文 8-UCA 内容降级为"8-UCA 理论参考",不再作为硬件预算主结果,最终报告
与"4 物理阵元硬件路线"两栏并列、严禁混淆;③实际 XYZ 未知,不假设
ULA/UCA/方阵,新增 sim/array_configs/actual_4elem_dualfeed_placeholder.json
由用户填坐标;④阵列代码 M/array_xyz_m 完全通用,禁止写死 8-UCA(T1 最高
优先);⑤T3 敏感性主报告优先 M=4 任意实际 XYZ,厂家坐标到手立即重跑;
