...
## 2026-08-14 - Codex - 四阵元 ULA 真实 GPS 波形与 FBSS 多维扫描

- 新增 GPS L1 C/A 原始复基带仿真：PRN 1 至 32 的 G1/G2 Gold 码、20 ms 导航比特、20.46 MHz 采样和逐 1 ms 多抽头相关；不再用两个独立窄带随机源代替 GNSS。
- 两端按同源功分建模，相干系数默认 1；馈线和传播路径共同决定码时延与 L1 载波相位，禁止将两者设成互不相关的自由量。
- 手机显示的是 C/N0，不是带内 SNR；读数可能覆盖约 10 至 40 dB-Hz 或更宽。`30 dB-Hz` 仅用于说明 1 ms 相关后约 0 dB SNR、原始单样点约 -43.1 dB，不是场景默认值或算法门限。正式实验应扫描 C/N0 × 相位 × 功率比 × 几何。
- 无噪声结构测试确认：相干双源使原始协方差秩 1；三阵元 FBSS 恢复两个有效特征值和两个正确角峰。
- 固定点实际 C/N0 约 29 dB-Hz：直接 MUSIC 错解，FBSS 默认同相只见一峰；12 相位仅 1 个通过。正式 C/N0 预扫扩展到实际约 9 至 49 dB-Hz，每档 3 次，在固定 90 度相位下 FBSS 成功率依次为 0/0/33/67/100/100/100%；这只是趋势预扫，不是统一门限。参考 40 dB-Hz 的相位扫描通过 9/12，直接 MUSIC 始终失败。
- 完整 360 度随机圆域严格成功率为 0：ULA 前后镜像、两路 C/N0 不平衡和相位病态共同作用。四阵元 ULA 当前只推荐为成熟 FBSS 基线/半平面样机，不作为停车场全方位最终定型。
- 完整报告见 `26_四阵元线阵真实GNSS波形与FBSS仿真报告.md`。

---

## 2026-08-13 - Codex - `2×2` SAGE/STAP 与 ULA+FBSS 第一阶段公平复现

- 新增真正的逐路径 SAGE 更新：减去其他径，在角度×时延×多普勒上最大化匹配似然，再更新复幅度；不是给旧 GLRT 改名。
- 复现论文结构参数：`2×2` 半波长方阵、`P=20`、`Cs=0.1 chip`、`20×1 ms`；100 次 Monte Carlo 已运行，但当前仅为固定俯仰角切片，未复现 CRB。
- 公平性修正一：SAGE 不再围绕真值角度搜索，而是全方位搜索。
- 公平性修正二：ULA+FBSS 使用全部 multicorrelator taps 形成协方差；Prompt-only 会错误隐藏接近 `1 chip` 的第二径。
- 公平性修正三：ULA 同时报无半平面先验与停车场半平面先验。前者因前后镜像为 0%，后者理想核为 100%，不能混为一个结果。
- 理想核下 `2×2` SAGE 总成功率 85.5%，但 `0.1 chip/0 dB` 仅 16.3%，存在路径交换/局部极值；`0.3 chip` 以上通过。
- NUC `192.168.114.204` 本轮连接超时，Phase A 实测核未取回。天线定型继续暂缓，完整报告见 `25_方阵SAGE_STAP与线阵FBSS公平复现报告.md`。

---

## 2026-08-13 - Codex - 方阵算法文献证据修正

- 用户指出“空间-时延 GLRT/ML + 协方差重构 + 置信度门控”缺少直接论文依据，质疑成立。
- 文献确认 GNSS 四阵元 `2×2` 方阵存在直接路线：SAGE/STAP multicorrelator 联合估计路径角度、时延、多普勒和幅度；另有 `2×2` MUSIC + 波束形成实测，但后者主要用于测角和抑制。
- 经典二维空间平滑对两个相干源的通用保证至少需要 `4×4` URA；不能声称四阵元 `2×2` 可直接靠标准二维平滑解决。
- 现有方阵 GLRT 脚本重新定性为“SAGE/STAP 联合模型启发的简化研究原型”；支撑约束重构不是独立检出器；门控阈值尚待真实负对照标定。
- 天线制作结论暂缓。先复现方阵 SAGE/STAP，并与 ULA+FBSS 在相同停车场场景和真实核上比较。完整证据见 `24_方阵GNSS相干双源分离文献证据审查.md`。

---

## 2026-08-13 - Codex - 四阵元方阵相干双源的处理边界

- `2×2` 方阵不能直接套用两源标准平移子阵平滑：可平移 `1×2` 子阵维数等于源数，MUSIC 噪声子空间为零；整个 `2×2` 又没有多个完整平移副本。
- 正确的四阵元路线是空间-时延联合 GLRT/ML，模板为 `a(theta) kron R(tau)`；码时延维度与空间维度共同提供可辨识性。
- 协方差重构必须由联合估计得到的支撑约束；它负责去相干后的 MUSIC 精化/显示，不能描述成从秩 1 原始协方差中独立恢复两源。
- 合成默认实验在完全相干、L5 `0.5 chip`、`-30/30 deg`、`-6 dB` 条件下恢复真值；结论暂不外推到未校准真实四通道硬件。
