...
3. **T3 流形扰动 + faithful 纹理合成 + 相位预算反推**：在 T1 生成器上加 (a) 逐元素复流形误差 ε(可设 3/5/10%)、(b) 通道相位漂移过程、(c) path0 用真实 A-only 纹理(复用 doc12/15 管线)——产出 G2/G3 的仿真预演包线,并输出**通道相位误差→0.5-chip 分离性能的敏感性曲线,反推 G0 的最终相位误差预算**(替换暂定 5° RMS)。**这是把 [理想仿真] 升级到 [纹理合成] 的关键任务。**
