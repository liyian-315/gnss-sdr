...
**v1.1 定点修订（2026-08-11,§13）:** 实际硬件定稿为 **4 物理陶瓷阵元×双馈(8 路 RF)**,非 8 独立空间位置 UCA;全文 8-UCA 内容降级为理论参考,硬件路线以 §13 为准。
...
**触发事实**：最终硬件确认为 **4 个物理陶瓷天线阵元,每阵元双馈,共 8 路 RF 进 Y790s**——不是 8 个独立空间位置的 UCA。设备可用窗口仅 **20 天**;最终实验环境为**地下停车场**,DAS 主动发射天线约每 **30–40 m** 一处。本节按冻结规则做定点修订,总体结构(路线 P/R、Gate 顺序、四态置信、证据等级)不变。
...
5. **场景注记(地下停车场)**:DAS 间距 30–40 m 意味着相邻发射点的相对延迟天然可达 **~1 chip 量级(L5 29.3 m/chip),角距大**——几何上比实验室 0.5 chip 更友好;但多反射/近场/遮挡环境的流形与纹理污染未知,**一切以实采数据为准**,不预支乐观结论。
...
- raw 仍为 8 路 `[sample, channel]`,`channel` = RX 通道号;metadata 必含逐通道映射表:
  `rx_mapping: [{rx_channel, element_id(0..3), feed_id("A"|"B")}, ...×8]`。
- 空间处理默认取每阵元**主馈(feed A)**构成 M=4;feed B 原始数据完整保留,仅作存档与未来研究,**禁止当第 5–8 个空间阵元进 steering/流形**。
...
- G0/校准 `G_m(f)` 仍按 8 路 RX 全测(两馈都测),但流形/估计只消费选定的 4 路。
