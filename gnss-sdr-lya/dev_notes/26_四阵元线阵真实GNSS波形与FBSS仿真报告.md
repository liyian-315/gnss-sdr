...
| `FIXED_RECEIVED_POWER_RATIO_DB` | 固定点阵列中心收到的 B/A 功率比；`0` 仅为公平资格基准，实测 B 弱 6 dB 时填 `-6`，`None` 才按传播模型计算 |
| `FIXED_ACTUAL_CN0_DB_HZ` | 固定点 A 路实际 C/N0；B 路为本值加上接收功率比 |
...
| `ULA_AXIS_DEG` | 线阵安装方向 |
| `SPATIAL_WAVE_MODEL` | `plane` 为标准远场 FBSS 基准；`spherical` 为精确球面波/有限距离压力测试 |
...
| `FIXED_DOA_MONTE_CARLO_RUNS` | 固定点 DOA-only 独立噪声重复数，默认 30 |
| `CN0_SWEEP_RUNS` | 每个 C/N0 点的重复数，预分析 30，正式曲线建议 100 |
...
不能机械沿用。每档 3 次只会得到 `0/33.3/66.7/100%` 四种粗糙比例，不能据此判断
检测率随 C/N0 是否单调。当前默认 30 次用于预分析，正式论文曲线建议 100 次，并同时
报告 Wilson 95% 置信区间。

### 6.5 等接收功率 DOA 资格基准

固定 A `(-10,0)` m、B `(10,0)` m、接收机 `(-10,5)` m，传播距离分别为 5 m 和
20.616 m，因此真实方向为 `270°` 和 `345.964°`，自由空间时延差为 15.616 m。若两端
发射功率相同，B 到达接收机后会弱 12.304 dB；这不是“等功率 DOA 基准”。脚本开启
`FIXED_RECEIVED_POWER_RATIO_DB=0` 后自动给 B 增加 12.304 dB，使两路到达幅度相同，
并把两路实际 C/N0 都设为 45 dB-Hz。输出明确标为 `FAIR_ALGORITHM_QUALIFICATION`：
它只回答公平条件下算法能否工作，**不是停车场性能**。与实物比较时必须把实测 B/A
接收功率比（例如 `-6 dB`）和 A 路实际 C/N0 填回参数。

标准远场平面波模型下，30 次独立热噪声结果为：

| 方法 | DOA-only 成功 | 角度 RMSE 中位数 | 角度 RMSE 95 分位 |
|---|---:|---:|---:|
| 直接 MUSIC | 0/30 | 36.57° | 71.93° |
| FBSS + MUSIC | 30/30 | 0.53° | 1.44° |

固定示例中 FBSS 输出 `270°/345.5°`。这证明在标准 FBSS 数据模型成立时，相干双源的
两个角度可以稳定恢复；直接 MUSIC 的失败也符合相干源协方差降秩理论。该阶段只用角度
门限验收，不让后续时延误差反向污染 DOA 判定。

### 6.6 ULA 镜像与球面波/有限距离模型边界

X 轴 ULA 的理想导向矢量只依赖方向余弦 `cos(theta)`，因此：

$$
\mathbf{a}(\theta)=\mathbf{a}(360^\circ-\theta).
$$

例如 `206.565°` 与 `153.435°` 的阵列响应完全相同。固定点已知两个发射机都位于接收机
Y 负半平面时，`KNOWN_HALF_PLANE="negative_y"` 可从镜像中选择 `180°~360°` 分支；
随机接收机跨越阵列两侧时，该先验不成立，四元 ULA 自身不能给出无歧义 360° 方位。

另一个独立边界是球面波/有限距离流形。当前 ULA 总孔径约 0.382 m，A 路距离仅 5 m，
精确球面波
在阵元间产生约 4.6° 的非线性相位弯曲。标准 FBSS 假设各平移子阵共享同一
Vandermonde 导向结构；切换 `SPATIAL_WAVE_MODEL="spherical"` 后，该结构被破坏。本次
5 次诊断中远场 FBSS 的 DOA 成功率从 5/5 降为 0/5。这个小样本只用于确认模型失配，
不是检测率统计。即便按传统天线远场公式 5 m 未必属于典型近场，MUSIC/FBSS 对几度
阵元相位失配也很敏感。真实停车场有限距离点需要球面波聚焦/有限距离阵列流形或现场
实测流形，不能把远场 30/30 直接外推到硬件。

因此实验顺序固定为：先在等到达功率、45 dB-Hz、15.616 m 时延下通过远场 DOA；再以
相同功率逐步降低 C/N0；然后固定高 C/N0，依次测试 B 路 `-3/-6/-10/-12 dB`。只有 DOA
稳定恢复两角后，才评价两路复相关时延。
