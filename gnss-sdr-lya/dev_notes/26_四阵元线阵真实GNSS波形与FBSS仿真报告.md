...
```bash
python dev_notes/sim/sweep_ula_finite_range_mismatch.py
```

该实验不是固定 20 m 停车场位置扫描，而是隔离流形失配的受控实验。它以固定接收机为
缩放中心，把 A/B 发射机方向向量同比例缩放，因此保持真实方位 `270°/345.964°`；通过
馈线差把有效路径差固定在 15.616 m；B/A 接收功率比固定 0 dB，两路实际 C/N0 固定
45 dB-Hz。唯一主要变化是发射源距离以及由此产生的球面波曲率。

阵元 $m$ 对信源 $s$ 的精确球面波相位与同方位平面波相位之差定义为：

$$
\epsilon_{s,m}=-\frac{2\pi}{\lambda}
\left(\left\|\mathbf{p}_s-(\mathbf{p}_r+\mathbf{r}_m)\right\|-R_s\right)
-\frac{2\pi}{\lambda}\mathbf{r}_m^{\mathsf T}\mathbf{u}_s.
$$

其中 $\mathbf{p}_s$ 是发射源位置，$\mathbf{p}_r$ 是阵列中心，$\mathbf{r}_m$ 是阵元
相对阵列中心的位置，$R_s=\|\mathbf{p}_s-\mathbf{p}_r\|$，$\mathbf{u}_s$ 是从阵列
指向信源的单位向量。去掉公共相位后，用 $\epsilon_{s,m}$ 在四阵元上的峰峰值表示流形
失配强度。

每个距离运行 30 次完整 102.3 MHz B2a data/pilot 波形与 100 ms 相关链，结果如下：

| A 路距离 | A 路相位失配峰峰值 | FBSS DOA 成功 | 成功率 | 角 RMSE 中位数 |
|---:|---:|---:|---:|---:|
| 2 m | 11.44° | 0/30 | 0% | 74.01° |
| 3 m | 7.64° | 0/30 | 0% | 76.99° |
| 5 m | 4.59° | 0/30 | 0% | 79.56° |
| 7.5 m | 3.06° | 7/30 | 23.3% | 4.27° |
| 10 m | 2.29° | 21/30 | 70.0% | 2.68° |
| 15 m | 1.53° | 25/30 | 83.3% | 1.79° |
| 20 m | 1.15° | 30/30 | 100% | 1.09° |
| 30 m | 0.76° | 30/30 | 100% | 1.09° |
| 50 m | 0.46° | 30/30 | 100% | 0.68° |

结果在本受控条件下总体单调：`2~5 m` 错误峰占主导，`7.5~15 m` 是噪声相关的过渡带，
`20 m` 后 30 次均通过。它说明标准 FBSS 对数度量级的阵元相位模型误差很敏感，但不能
据此宣布“停车场 20 m 外一定可用”。该边界仍依赖阵列孔径、入射角、C/N0、B/A 功率
比、角间隔和校准误差。真正对应实物的下一层验证必须固定两发射天线间距 20 m，按接收
机位置计算精确球面波，并填入实测功率比和 C/N0。

输出位于 `dev_notes/sim/results/ula_b2a_finite_range_sweep/`：逐次 CSV 保存 270 次角度，
汇总 CSV 保存 Wilson 95% 区间，PNG 同时画成功率、角度 RMSE 和阵元相位失配。
