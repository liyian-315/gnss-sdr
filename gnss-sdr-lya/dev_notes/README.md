...
**当前阶段：`Phase A 单源参考库(L5)已建 ✅ → Phase B 静态快照分离墙已量化 🟡 → Track B 移动轨迹 DP + 相关域 EKF faithful synthetic 已通过 🟡 → 真实移动采集待验证 🔴`（Phase B 全程与权威目标定义见 `11`；Track B 见 `13`/`14`）**
...
- **较强目标**：推进到 **0.3 chip≈8.8 m**；覆盖 −10 dB 第二径、多数相位可检、RMSE≤0.05–0.1 chip、明显优于固定理想核 MEDLL、**能给置信区间 + 失败告警**。
- **高挑战/研究**：**0.1–0.2 chip≈2.9–5.9 m**，不强求全成功；**明确报告哪些相位/功率比可分、哪些多解、算法如何识别"自己不确定"**——超分辨里"可靠地报不可分"比硬输出一个错误第二径更有价值。
- **实测现状（真实 PRN23 30/60/90 m，独立钟）**：绝对延迟标定线 **slope≈1.00 / 固定偏移≈−6.5 m**（配对增量可信，绝对值带钟/线缆 DC）；幅度比**仅在相位分集足够的 run 可信**；载波–码交叉校验比值在**合并区不可靠**（探针被主瓣污染）。→ 正给 `fit_windowed_twosource.py` 加 **相位覆盖门禁 + Theil-Sen 稳健斜率 + 分集自校**，把"分不开/不确定"变成显式判据（虚警控制的关键）。
