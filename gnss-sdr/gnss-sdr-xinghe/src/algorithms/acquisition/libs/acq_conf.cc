    ...
    const std::string &role, double chip_rate, double opt_freq)
{
    ...
    second_peak_exclusion_chips = configuration->property(
        role + ".second_peak_exclusion_chips", second_peak_exclusion_chips);
    if (second_peak_exclusion_chips < 0.0F)
        {
            LOG(WARNING) << "Parameter second_peak_exclusion_chips cannot be negative. Setting it to 1.0";
            second_peak_exclusion_chips = 1.0F;
        }
