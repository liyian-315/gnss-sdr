...
float pcps_acquisition_fine_doppler_cc::compute_CAF()
{
    ...
    const auto exclusion_samples = static_cast<uint32_t>(std::ceil(
        d_acq_params.second_peak_exclusion_chips *
        static_cast<float>(d_acq_params.samples_per_chip)));
    const auto fft_size = static_cast<uint32_t>(d_fft_size);
    const auto exclude_peak = [&](uint32_t peak_index) {
        for (uint32_t candidate_index = 0; candidate_index < fft_size; ++candidate_index)
            {
                const auto linear_distance = candidate_index > peak_index ? candidate_index - peak_index : peak_index - candidate_index;
                const auto circular_distance = std::min(linear_distance, fft_size - linear_distance);
                if (circular_distance <= exclusion_samples)
                    {
                        d_grid_data[index_doppler][candidate_index] = 0.0F;
                    }
            }
    };

    // Exclude the strongest peak's main lobe and find the next independent
    // peak in the same Doppler bin.
    exclude_peak(index_time);
    ...
    float selectedPeak = firstPeak;
    float comparisonPeak = secondPeak;
    if (d_gnss_synchro->Signal_Path == 1U)
        {
            selectedPeak = secondPeak;
            index_time = tmp_intex_t;
            exclude_peak(index_time);
            volk_gnsssdr_32f_index_max_32u(&tmp_intex_t, d_grid_data[index_doppler].data(), d_fft_size);
            comparisonPeak = d_grid_data[index_doppler][tmp_intex_t];
        }

    // 5- Compute the test statistics and compare to the threshold
    d_test_statistics = comparisonPeak > std::numeric_limits<float>::epsilon() ? selectedPeak / comparisonPeak : 0.0F;
    ...
    gr_vector_void_star &output_items)
{
    ...
    switch (d_state)
        ...
        case 4:  // Positive_Acq
            ...
            DLOG(INFO) << "signal path " << d_gnss_synchro->Signal_Path;
            d_positive_acq = 1;
            d_active = false;
            // Record results to file if required
            if (d_dump and d_channel == d_dump_channel)
...
void pcps_acquisition_fine_doppler_cc::dump_results(int effective_fft_size)
{
    ...
    if (matfp == nullptr)
    ...
    else
        {
            ...
            write_matlab_var<1, uint32_t>("signal_path", d_gnss_synchro->Signal_Path, matfp, dims);
