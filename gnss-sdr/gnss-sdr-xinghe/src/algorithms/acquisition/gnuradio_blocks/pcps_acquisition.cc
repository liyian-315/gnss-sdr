...
pcps_acquisition::pcps_acquisition(const Acq_Conf& conf_)
    : acquisition_impl_interface("pcps_acquisition",
      ...
      d_second_peak_exclusion_samples(static_cast<uint32_t>(std::ceil(
          conf_.second_peak_exclusion_chips * static_cast<float>(conf_.samples_per_chip)))),
...
void pcps_acquisition::log_acquisition(const AcquisitionResult& result) const
{
    DLOG(INFO) << (result.positive_acq ? "positive" : "negative") << " acquisition"
               ...
               << ", selected peak magnitude " << result.peak_magnitude
...
void pcps_acquisition::dump_results(const AcquisitionResult& result)
{
    ...
    if (matfp == nullptr)
    ...
    else
        {
            ...
            write_matlab_var<1>("selected_peak_magnitude", result.peak_magnitude, matfp, dims_1d);
...
{
    std::vector<uint32_t> excluded_code_phases;
    const auto selected_peak = find_selected_path_peak(num_doppler_bins, excluded_code_phases);
    auto result = make_result(selected_peak, doppler_max, doppler_step);

    if (!d_step_two)
        {
            const auto index_opp = (selected_peak.doppler_index + d_num_doppler_bins / 2) % d_num_doppler_bins;
    ...
    if (d_input_power < std::numeric_limits<float>::epsilon())
        ...
        {
            result.test_statistics = selected_peak.magnitude / d_input_power;
        }
...
pcps_acquisition::AcquisitionResult pcps_acquisition::first_vs_second_peak_statistic(uint32_t num_doppler_bins, int32_t doppler_max, int32_t doppler_step)
{
    ...
    // The next peak is chosen outside the configured code-phase exclusion region.

    std::vector<uint32_t> excluded_code_phases;
    const auto selected_peak = find_selected_path_peak(num_doppler_bins, excluded_code_phases);
    auto result = make_result(selected_peak, doppler_max, doppler_step);

    // Compare the selected path with the next independent peak in the same
    // Doppler bin. For path 0 this preserves the original first/second-peak
    // statistic; for path 1 it becomes a second/third-peak statistic.
    const auto comparison_peak = find_highest_peak(
        num_doppler_bins, excluded_code_phases, static_cast<int32_t>(selected_peak.doppler_index));

    // Compute the test statistics and compare to the threshold
    if (comparison_peak.magnitude > std::numeric_limits<float>::epsilon())
        {
            result.test_statistics = selected_peak.magnitude / comparison_peak.magnitude;
        }
...
bool pcps_acquisition::code_phase_is_excluded(uint32_t index_time,
    const std::vector<uint32_t>& excluded_code_phases) const
{
    for (const auto excluded_index : excluded_code_phases)
        {
            const auto linear_distance = index_time > excluded_index ? index_time - excluded_index : excluded_index - index_time;
            const auto circular_distance = std::min(linear_distance, d_effective_fft_size - linear_distance);
            if (circular_distance <= d_second_peak_exclusion_samples)
                {
                    return true;
                }
        }
...
pcps_acquisition::AcquisitionPeak pcps_acquisition::find_highest_peak(
    uint32_t num_doppler_bins,
    const std::vector<uint32_t>& excluded_code_phases,
    int32_t only_doppler_bin) const
{
    AcquisitionPeak peak;
    const auto first_bin = only_doppler_bin >= 0 ? static_cast<uint32_t>(only_doppler_bin) : 0U;
    const auto last_bin = only_doppler_bin >= 0 ? first_bin + 1U : num_doppler_bins;

    for (uint32_t doppler_index = first_bin; doppler_index < last_bin; ++doppler_index)
        {
            for (uint32_t index_time = 0; index_time < d_effective_fft_size; ++index_time)
                {
                    const auto magnitude = d_magnitude_grid[doppler_index][index_time];
                    if (magnitude > peak.magnitude && !code_phase_is_excluded(index_time, excluded_code_phases))
                        {
                            peak.magnitude = magnitude;
                            peak.doppler_index = doppler_index;
                            peak.index_time = index_time;
                        }
                }
        }
    return peak;
...
pcps_acquisition::AcquisitionPeak pcps_acquisition::find_selected_path_peak(
    uint32_t num_doppler_bins,
    std::vector<uint32_t>& excluded_code_phases) const
{
    AcquisitionPeak selected_peak;
    const auto signal_path = std::min(d_gnss_synchro->Signal_Path, 1U);
    for (uint32_t path = 0; path <= signal_path; ++path)
        {
            selected_peak = find_highest_peak(num_doppler_bins, excluded_code_phases);
            excluded_code_phases.push_back(selected_peak.index_time);
        }
    return selected_peak;
}


pcps_acquisition::AcquisitionResult pcps_acquisition::make_result(
    const AcquisitionPeak& peak, int32_t doppler_max, int32_t doppler_step) const
{
    AcquisitionResult result;
    result.index_time = peak.index_time;
    result.peak_magnitude = peak.magnitude;
    if (!d_step_two)
        {
            result.doppler = -doppler_max + d_doppler_center + doppler_step * static_cast<int32_t>(peak.doppler_index);
        }
    else
        {
            result.doppler = static_cast<int32_t>(d_doppler_center_step_two +
                                                  (static_cast<float>(peak.doppler_index) - static_cast<float>(floor(d_num_doppler_bins_step2 / 2.0))) *
